...
def main():
    ...
    for oeb_val, desc in OEB_DESCRIPTIONS.items():        st.sidebar.markdown(f"**OEB {oeb_val}:** {desc.split(":")[1].split("(")[0].strip()}")
    st.sidebar.markdown("---")
            ...
            if not current_smiles_for_pred:
                ...
                if features is None:
                    ...
                    if hasattr(model, "predict_proba"):
                        ...
                        # Ensure decision_scores is 2D for softmax
                        if decision_scores.ndim == 1:
                            decision_scores = decision_scores.reshape(1, -1)
                        probs = softmax(decision_scores, axis=1)[0]

                    # This block handles cases where the number of predicted probabilities
                    # does not match the number of OEB descriptions.
                    # This was present in oeb_predictor_app6.py and is crucial for consistency.
                    if len(probs) != len(OEB_DESCRIPTIONS):
                        st.warning(f"Predicted probabilities length ({len(probs)}) does not match OEB_DESCRIPTIONS length ({len(OEB_DESCRIPTIONS)}). Adjusting.")
